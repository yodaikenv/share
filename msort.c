 ...
 * in Art of Programing Volume 3 and includes
 * 1. arrays have an extra element so they can start at 1 which is 
 * 	the numbering scheme Knuth uses 
...
int
testsort (sort_t * x, unsigned int n)
{
  int i;
  for (i = 1; i < n; i++)
    {
...
void
initarray (sort_t * x, unsigned int n)
{
  int i;
  for (i = 0; i <= n; i++)
    x[i] = (sort_t) random ();
